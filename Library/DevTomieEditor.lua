...
[27] = function() 
    ...
    local ImportGlobals return (function(...) 
        ...
        function Element:New(Idx, Config)
            ...
            local StartMousePosition, StartDotPosition, DragInput
            ...
            local function UpdateSlider(Input)
                local Delta = Input.Position.X - SliderRail.AbsolutePosition.X
                local NewScale = math.clamp(Delta / SliderRail.AbsoluteSize.X, 0, 1)
                local NewDotPosition = UDim2.new(NewScale, -7, 0.5, 0)
                SliderDot.Position = NewDotPosition
                SliderFill.Size = UDim2.new(NewScale, 0, 1, 0)
                local Value = Slider.Min + (Slider.Max - Slider.Min) * NewScale
            ...
            -- Synchronize TextBox input with slider value
            SliderDisplay.FocusLost:Connect(function(enterPressed)
                local Value = tonumber(SliderDisplay.Text) or Slider.Min
