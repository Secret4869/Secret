...
function Element:New(Idx, Config)
    ...
    local function RecalculateListPosition()
        DropdownHolderCanvas.Position = UDim2.fromOffset(DropdownInner.AbsolutePosition.X - 1, DropdownInner.AbsolutePosition.Y + 35)
    end
    ...
    local function HandleButtonInput(ButtonLabel, Button, Table, Value)
        ButtonLabel.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                local TouchStartTime = tick()

                ButtonLabel.InputEnded:Connect(function(EndInput)
                    if EndInput.UserInputType == Enum.UserInputType.Touch and (tick() - TouchStartTime) > 0.2 then
                        -- Touch moved, likely a scroll gesture
                        return
                    end
        ...
        if Textbox.Frame.Frame.TextBox.Text ~= "" then
            ...
            for Idx, Value in pairs(Values) do
                ...
                if string.find(string.lower(Value), SearchText) then
                    ...
                    HandleButtonInput(ButtonLabel, Button, Table, Value)
